...
class node {
    ...
    public:
        ...
        virtual void set_table(unsigned int dst, unsigned int cnt, pair<unsigned int, double> ans) {}
        virtual unsigned int print_table_size(unsigned int dst) {return 1;}
        virtual unsigned int print_counter(unsigned int dst) {return 0;}
        virtual unsigned int print_table(unsigned int dst) {return 0;}
        virtual unsigned int print_sdn_table(unsigned int dst, unsigned int cnt) {return 0;} // 有vector
        virtual double print_sdn_portion(unsigned int dst, unsigned int cnt) {return 0;}
...
class TRA_switch: public node {
    ...
    public:
        ...
        map<unsigned int, unsigned int> table; // routing table
        map<unsigned int, unsigned int> cnt_table; // 存hop
        unsigned int preid; // parent
        unsigned int print_counter(unsigned int dst) override {return cnt_table[dst];}
        unsigned int print_table(unsigned int dst) override {return table[dst];}
...
class SDN_switch: public node {
    ...
    public:
        ...
        map<unsigned int, vector<pair<unsigned int, double>>> table; // routing table
        map<unsigned int, unsigned int> cnt_table; // 存hop
        unsigned int preid;
        double portion;
        // 把node next的portion存入
        void set_table(unsigned int dst, unsigned int cnt, pair<unsigned int, double> ans) override {if(cnt==0) table[dst][cnt] = ans; else table[dst].push_back(ans);}
        unsigned int print_table_size(unsigned int dst) override {return table[dst].size();}
        unsigned int print_counter(unsigned int dst) override {return cnt_table[dst];}
        unsigned int print_sdn_table(unsigned int dst, unsigned int cnt) override {return table[dst][cnt].first;}
        double print_sdn_portion(unsigned int dst, unsigned int cnt) override {return table[dst][cnt].second;}
        // please define recv_handler function to deal with the incoming packet
        virtual void recv_handler (packet *p);
        ...
        class SDN_switch_generator : public node_generator{
                ...
                // this constructor is only for sample to register this node type
                SDN_switch_generator() { /*cout << "TRA_switch registered" << endl;*/ register_node_type(&sample); }
            protected:
                virtual node * generate(unsigned int _id){ /*cout << "TRA_switch generated" << endl;*/ return new SDN_switch(_id); }
            public:
...
class SDN_controller: public node {
    ...
    public:
        ...
        map<unsigned int, unsigned int> _dst_id; // 存dst id
        // please define recv_handler function to deal with the incoming packet
        virtual void recv_handler (packet *p);
        ...
        class SDN_controller_generator : public node_generator{
                ...
                // this constructor is only for sample to register this node type
                SDN_controller_generator() { /*cout << "TRA_switch registered" << endl;*/ register_node_type(&sample); }
            protected:
                virtual node * generate(unsigned int _id){ /*cout << "TRA_switch generated" << endl;*/ return new SDN_controller(_id); }
            public:
...
void TRA_switch::recv_handler (packet *p){
    ...
    if (p->type() == "TRA_ctrl_packet") { // the switch receives a packet from the controller
        ...
        bool pass = false;
        preid = p3->getHeader()->getPreID();
        if(table.find(p3->getHeader()->getSrcID()) == table.end()){ // table內找不到ID
            table[p3->getHeader()->getSrcID()] = preid; // 存入table
            cnt_table[p3->getHeader()->getSrcID()] = l3->getCounter(); // 存hop
            pass = true;
        }
        else{
            if(l3->getCounter() == cnt_table[p3->getHeader()->getSrcID()]){ // 路徑長一樣
                if(preid < table[p3->getHeader()->getSrcID()]){ // 選id小的
                    table[p3->getHeader()->getSrcID()] = preid;
                    pass = true;
                }
            }
            else  if(l3->getCounter() < cnt_table[p3->getHeader()->getSrcID()]){ // 路徑長較小
                    table[p3->getHeader()->getSrcID()] = preid; 
                    cnt_table[p3->getHeader()->getSrcID()] = l3->getCounter();
                    pass = true;
            }
        }
        // 設封包內容
        ...
        TRA_data_packet *pkt = dynamic_cast<TRA_data_packet*>(p);
        if(pkt->getHeader()->getDstID() == getNodeID())  return;

        if(table.find(pkt->getHeader()->getDstID()) != table.end()){
            pkt->getHeader()->setPreID ( getNodeID() );
            pkt->getHeader()->setNexID ( table[pkt->getHeader()->getDstID()] );
            send_handler(pkt);
        }
        else    return;
    }
...
void SDN_switch::recv_handler (packet *p){
    if(p->type() == "TRA_ctrl_packet"){
        ...
        preid = p3->getHeader()->getPreID();
        if(table.find(p3->getHeader()->getSrcID()) == table.end()){ // table內找不到ID
            table[p3->getHeader()->getSrcID()].push_back(make_pair(preid, 1.0)); // 存入table
            cnt_table[p3->getHeader()->getSrcID()] = l3->getCounter(); // 存hop
            pass = true;
        }
        else{
            if(l3->getCounter() == cnt_table[p3->getHeader()->getSrcID()]){ // 路徑長一樣
                if(preid < table[p3->getHeader()->getSrcID()][0].first){ // 選id小的
                    table[p3->getHeader()->getSrcID()][0].first = preid;
                    pass = true;
                }
            }
            else  if(l3->getCounter() < cnt_table[p3->getHeader()->getSrcID()]){ // 路徑長較小
        ...
        if(table.find(pkt->getHeader()->getDstID()) != table.end()){ // 在table
            for(unsigned int i=0; i<table[pkt->getHeader()->getDstID()].size(); i++){
                pkt->setSize(pkt->getSize() * table[pkt->getHeader()->getDstID()][i].second); // 設定portion
                pkt->getHeader()->setPreID ( getNodeID() ); // 設定preID
                pkt->getHeader()->setNexID ( table[pkt->getHeader()->getDstID()][i].first); // 設定nextID
                send_handler(pkt);
            }
        }
        else return;
...
void SDN_controller::recv_handler (packet *p){
    if(p->type() == "TRA_ctrl_packet"){
        TRA_ctrl_packet *p3 = dynamic_cast<TRA_ctrl_packet*> (p);
        if(_dst_id.find(p3->getPacketID()) == _dst_id.end())
            _dst_id[p3->getPacketID()] = p3->getHeader()->getSrcID();
    }
    else if(p->type() == "SDN_invoke_packet"){
        SDN_invoke_packet *pkt = dynamic_cast<SDN_invoke_packet*>(p);
        vector<vector<double>> v = (dynamic_cast<SDN_invoke_payload*>(pkt->getPayload()))->getTrafficMatrix();
        
        vector<unsigned int> src_node, dst_node;
        for(unsigned int i=0; i<getNodeID(); i++){
            for(unsigned int j=0; j<getNodeID(); j++){
                if(v[i][j] != 0){
                    src_node.push_back(i);
                    dst_node.push_back(j);
                }
            }
        }

        for(unsigned int idx=0; idx<src_node.size(); idx++){
            unsigned int tmp_node = src_node[idx];
            while(tmp_node != dst_node[idx]){
                if(node::id_to_node(tmp_node)->type() == "TRA_switch"){
                    v[node::id_to_node(tmp_node)->print_table(dst_node[idx])][0] = 1;
                    tmp_node = node::id_to_node(tmp_node)->print_table(dst_node[idx]);
                }
                else{
                    v[node::id_to_node(tmp_node)->print_sdn_table(dst_node[idx], 0)][0] = 1;
                    tmp_node = node::id_to_node(tmp_node)->print_sdn_table(dst_node[idx], 0);
                }
            }
        }

        vector<unsigned int> access_node;
        for(map<unsigned int, unsigned int>::iterator iter = _dst_id.begin(); iter != _dst_id.end(); iter++){
            for(unsigned int i=0; i<getNodeID(); i++){
                access_node.clear();
                access_node.shrink_to_fit();
                if(node::id_to_node(i)->type() == "SDN_switch"){
                    const map<unsigned int,bool> &nblist = node::id_to_node(i)->getPhyNeighbors();
                    for(map<unsigned int, bool>::const_iterator it = nblist.begin(); it != nblist.end(); it++){
                        if(node::id_to_node(it->first)->type() == "TRA_switch" && node::id_to_node(it->first)->print_counter((*iter).second) <= node::id_to_node(i)->print_counter((*iter).second))
                            access_node.push_back(it->first);
                        else if(node::id_to_node(it->first)->type() == "SDN_switch" && node::id_to_node(it->first)->print_counter((*iter).second) < node::id_to_node(i)->print_counter((*iter).second))
                            access_node.push_back(it->first);
                    }
                    if(access_node.size() == 0) 
                        node::id_to_node(i)->set_table((*iter).second, 0, make_pair(i, 1.0));
                    else{
                        int counter = 0;
                        for(unsigned int i=1; i<access_node.size(); i++){
                            if(v[access_node[i]][(*iter).second] >= 1)
                                counter++;
                        }
                        for(unsigned int in=0; in<access_node.size(); in++){
                            if(v[access_node[in]][(*iter).second] == 0 || in == 0)
                                node::id_to_node(i)->set_table((*iter).second, in, make_pair(access_node[in], 1.0/(access_node.size()-counter)));
                            else
                                node::id_to_node(i)->set_table((*iter).second, in, make_pair(access_node[in], 0));
                        }
                    }
                }
            }
        ...
        for(map<unsigned int, unsigned int>::iterator iter = _dst_id.begin(); iter != _dst_id.end(); iter++){
            for(unsigned int i=0; i<getNodeID(); i++){
                if(node::id_to_node(i)->type() == "SDN_switch"){
                    for(unsigned int j=0; j<node::id_to_node(i)->print_table_size((*iter).second); j++)
                        SDN_ctrl_packet_event(getNodeID(), i, (*iter).second, node::id_to_node(i)->print_sdn_table((*iter).second, j), node::id_to_node(i)->print_sdn_portion((*iter).second, j));    
                }
            }
        }
    }
...
int main(){
    ...
    unsigned int nodes_num, sdn_node_num, dsts_num, links_num, pairs_num, simtime, invoketime;
    vector<unsigned int> sdn_list, dst_list;
    map<unsigned int, unsigned int> dst_and_boardcasttime;
    cin >> nodes_num >> sdn_node_num >> dsts_num >> links_num >> pairs_num >> simtime >> invoketime;
    
    unsigned int sdn_node_tmp;
    for(unsigned int i=0; i<sdn_node_num; i++){
        cin >> sdn_node_tmp;
        sdn_list.push_back(sdn_node_tmp);
    }

    unsigned int dst_node_tmp;
    for(unsigned int i=0; i<dsts_num; i++){
        cin >> dst_node_tmp;
        dst_list.push_back(dst_node_tmp);
    }

    unsigned int dst_tmp, boardcasttime_tmp;
    for(unsigned int i=0; i<dsts_num; i++){
        cin >> dst_tmp >> boardcasttime_tmp;
        dst_and_boardcasttime[dst_tmp] = boardcasttime_tmp;
    }
    ...
    for (unsigned int id=0; id<nodes_num; id++){
        ...
        if(find(sdn_list.begin(), sdn_list.end(), id) != sdn_list.end())
    ...
    for(unsigned int i=0; i<sdn_node_num; i++){
        node::id_to_node(sdn_list[i])->add_phy_neighbor(con_id);
        node::id_to_node(con_id)->add_phy_neighbor(sdn_list[i]);  
    }
    //
    ////////

    // node 0 broadcasts a msg with counter 0 at time 100
    for(auto i: dst_and_boardcasttime){
        TRA_ctrl_packet_event(i.first, i.second);
    }
    // TRA_ctrl_packet_event(0, 100);
    // 1st parameter: the source; the destination that want to broadcast a msg with counter 0 (i.e., match ID)
    // 2nd parameter: time (optional)
    // 3rd parameter: msg for debug information (optional)
    vector<unsigned int> flowid, src, dst, starttime;
    unsigned int flow_id_tmp, flow_src_tmp, flow_dst_tmp, starttime_tmp;
    vector<double> flowsize;
    double flowsize_tmp;
    vector<vector<double>> traffic_matrix;
    // initial traffic_matrix
    traffic_matrix.resize(con_id);
    for(unsigned int i=0; i<con_id; i++)
        traffic_matrix[i].resize(con_id);

    for(unsigned int i=0; i<con_id; i++){
        for(unsigned int j=0; j<con_id; j++)
            traffic_matrix[i][j] = 0;
    }

    for(unsigned int i=0; i<pairs_num; i++){
        cin >> flow_id_tmp >> flow_src_tmp >> flow_dst_tmp >> flowsize_tmp >> starttime_tmp;
        flowid.push_back(flow_id_tmp);
        src.push_back(flow_src_tmp);
        dst.push_back(flow_dst_tmp);
        flowsize.push_back(flowsize_tmp);
        starttime.push_back(starttime_tmp);
        traffic_matrix[flow_src_tmp][flow_dst_tmp] = flowsize_tmp;
    } 
    ...
    SDN_invoke_packet_event(con_id, traffic_matrix, invoketime); 
    // 1st parameter: the controller id
    // 2nd parameter: time (optional)
    // 3rd parameter: msg for debug information (optional)   
    
    // SDN_ctrl_packet_event(con_id, 14, 0, 5, 0.3, invoketime);
    // SDN_ctrl_packet_event(con_id, 2, 0, 11, 0.5, invoketime);
    // 1st parameter: the controller id
    ...
    for(unsigned int i=0; i<pairs_num; i++){
        data_packet_event(src[i], dst[i], flowsize[i], starttime[i]);
    }
    ...
    // upgraded node ID list (SDNlist)
    for(unsigned int i=0; i<sdn_node_num; i++)
        cout << sdn_list[i] << " ";
    cout << endl; 
    for(unsigned int i=0; i<nodes_num; i++){
        cout << i << endl;
        for(unsigned int j=0; j<dsts_num; j++){
            if(node::id_to_node(i)->type() == "TRA_switch"){
                cout << dst_list[j] << " " << node::id_to_node(i)->print_table(dst_list[j]);
            }
            else{
                cout << dst_list[j] << " " << node::id_to_node(i)->print_sdn_table(dst_list[j], 0) << " " << node::id_to_node(i)->print_sdn_portion(dst_list[j], 0)*100 << "%" << " ";
                for(unsigned int k=1; k<node::id_to_node(i)->print_table_size(dst_list[j]); k++){
                    cout << node::id_to_node(i)->print_sdn_table(dst_list[j], k) << " " << node::id_to_node(i)->print_sdn_portion(dst_list[j], k)*100 << "%" << " ";  
                }
            }
            cout << endl;
        }
