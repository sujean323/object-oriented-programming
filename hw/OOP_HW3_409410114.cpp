...
//     node::node_generator::print(); // print all registered nodes
    ...
    unsigned int nodes_num, dsts_num, links_num, pairs_num, simtime, sdn_ctrltime, budget, dst_tmp, boardcasttime;
    vector<unsigned int> dst_list;
    // map<int, int> dst_and_boardcasttime;
    cin >> nodes_num >> dsts_num >> links_num >> pairs_num >> simtime >> sdn_ctrltime >> budget;
    for (unsigned int i=0; i<dsts_num; i++){
        cin >> dst_tmp;
        dst_list.push_back(dst_tmp);
    }
    int nodeid, upgradecost;
    int nodes_info[nodes_num][3];
    for(unsigned int i=0; i<nodes_num; i++){
        cin >> nodeid >> upgradecost;
        nodes_info[i][0] = nodeid;
        nodes_info[i][1] = upgradecost;
        nodes_info[i][2] = -1;
        if(std::find(dst_list.begin(), dst_list.end(), nodeid) != dst_list.end()){
            cin >> boardcasttime;
            nodes_info[i][2] = boardcasttime;
        }
    }
    ...
    for(unsigned int i=0; i<nodes_num; i++){
        if(nodes_info[i][2] != -1)
            TRA_ctrl_packet_event(i, nodes_info[i][2]);
    }
