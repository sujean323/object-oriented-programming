...
class node {
    ...
    public:
        ...
        virtual unsigned int print_table(unsigned int dst) = 0;
...
class TRA_switch: public node {
    ...
    public:
        ...
        string type() { return "TRA_switch"; }
         unsigned int print_table(unsigned int dst) override {return table[dst].first;}
        map<unsigned int, pair<unsigned int, unsigned int>> table;
        pair<unsigned int, unsigned int> preid_cnt;
...
void TRA_switch::recv_handler (packet *p){
    ...
    if (p->type() == "TRA_ctrl_packet") { // the switch receives a packet from the controller
        ...
        // cout << "NodeID: " << getNodeID() << " send or not: " << send_or_not[p3->getHeader()->getSrcID()] << endl;
        bool pass = false;
        preid_cnt.first = p3->getHeader()->getPreID();
        preid_cnt.second = l3->getCounter();
        if(table.find(p3->getHeader()->getSrcID()) == table.end()){
            table[p3->getHeader()->getSrcID()] = preid_cnt;
            pass = true;
        }
        else{
            if(preid_cnt.second == table[p3->getHeader()->getSrcID()].second){
                if(preid_cnt.first < table[p3->getHeader()->getSrcID()].first){
                    table[p3->getHeader()->getSrcID()] = preid_cnt;
                    pass = true;
                }
            }
            else  if(preid_cnt.second < table[p3->getHeader()->getSrcID()].second){
                    table[p3->getHeader()->getSrcID()] = preid_cnt;
                    pass = true;
            }
        }
        ...
        if(p3->getHeader()->getDstID() == getNodeID())  return;

        if(table.find(p3->getHeader()->getDstID()) != table.end()){
            p3->getHeader()->setPreID ( getNodeID() );
            p3->getHeader()->setNexID ( table[p3->getHeader()->getDstID()].first );
            // p3->getHeader()->setNexID ( BROCAST_ID );
            // p3->getHeader()->setDstID ( BROCAST_ID );
            send_handler(p3);
        }
    ...
    // getNodeID() re.turns the id of the current node
...
int main()
{
    ...
    unsigned int nodes_num, dsts_num, links_num, pairs_num, simtime, dst_tmp, boardcasttime_tmp;
    vector<unsigned int> dst_list;
    map<int, int> dst_and_boardcasttime;
    cin >> nodes_num >> dsts_num >> links_num >> pairs_num >> simtime;
    for (unsigned int i=0; i<dsts_num; i++){
        cin >> dst_tmp;
        dst_list.push_back(dst_tmp);
    }
    for(unsigned int i=0; i<dsts_num; i++){
    ...
    int flowid, src, dst, flowsize, starttime;
    for(unsigned int i=0; i<pairs_num; i++){
        cin >> flowid >> src >> dst >> flowsize >> starttime;
        data_packet_event(src, dst, starttime);
    }
    ...
    for(unsigned int i=0; i<nodes_num; i++){
        ...
        for(unsigned int j=0; j<dsts_num; j++){
            cout << dst_list[j] << " " << node::id_to_node(i)->print_table(dst_list[j]) << endl;
        }
    }
    return 0;
}
